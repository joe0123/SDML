...
if __name__ == '__main__':
    ...
    repeated = RepeatVector(decoder_seq_len)(encoder_output)
    decoder_nosig_input = Concatenate()([decoder_embedding, repeated])
    decoder_gru = GRU(2 * HID_DIM, return_sequences=True, return_state=True, dropout=0.1)
    decoder_output, _ = decoder_gru(decoder_nosig_input, initial_state=state_h)
    ...
    decoder_nosig_input = Concatenate()([decoder_embedding, repeated])
    decoder_output, decoder_state = decoder_gru(decoder_nosig_input, initial_state=decoder_state_input)
    decoder_output = decoder_dense(decoder_output)
    decoder = Model([decoder_input, decoder_state_input, encoded], [decoder_output, decoder_state])
    ...
    for seq_index in range(len(encoder_input_data)):
        ...
        while True:
            prob_out, states_value = decoder.predict([new_input, states_value, feature])
