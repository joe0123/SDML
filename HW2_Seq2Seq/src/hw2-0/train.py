...
if __name__ == "__main__":
    ...
    encoder_embedding = OneHot(input_dim = voc_size, input_length = seq_len)(encoder_input)
    encoder_output, forward_h, backward_h = Bidirectional(GRU(HID_DIM, return_state=True, dropout=0.1))(encoder_embedding)
    state_h = Concatenate()([forward_h, backward_h])

    decoder_input = Input(shape=(seq_len - 1, ))
    decoder_embedding = OneHot(input_dim = voc_size, input_length = seq_len - 1)(decoder_input)
    repeated = RepeatVector(seq_len - 1)(encoder_output)
    decoder_all_input = Concatenate()([decoder_embedding, repeated])
